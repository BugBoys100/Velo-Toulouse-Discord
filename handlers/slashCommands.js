			...
			if(config.loadSlashsGlobal){
				...
				client.guilds.cache.map(g => g).forEach((guild) => {
					try{
						...
						.then(slashCommandsData => {
							console.log(`${slashCommandsData.size} slashCommandes ${`(Avec ${slashCommandsData.map(d => d.options).flat().length} Sous Commandes)`.green} Chargées pour : ${`${guild.name}`.underline}\n`.brightGreen); 
						}).catch((e)=>console.log(e));
					}catch (e){
