...
module.exports = {
    name: "suppfavoris", //the command name for execution & for helpcmd [OPTIONAL]
    category: "Vélib", //the command category for helpcmd [OPTIONAL]
    aliases: ["favsupp", "suppfav", "suppfavori", "favorissupp", "favorisupp"], //the command aliases for helpcmd [OPTIONAL]
    cooldown: 5, //the command cooldown for execution & for helpcmd [OPTIONAL]
    usage: "suppfavoris", //the command usage for helpcmd [OPTIONAL]
    ...
    run: async (client, message, args, plusArgs, cmdUser, text, prefix) => {
        try {
            ...
            try {
                ...
                for (var i = 0; i < lg_fav; i++) { premier_etage.push(new MessageButton().setCustomId('del' + db[message.author.id][i]).setLabel('Supprimer ' + db[message.author.id][i]).setStyle('DANGER')) }
